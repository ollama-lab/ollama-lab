...
- `ollama-rest` callback APIs require an explicit callback argument even for non-stream calls (`ollama.chat(&req, None)`), and `ChatResponse.message` is optional; unwrap defensively before reading `content`.
