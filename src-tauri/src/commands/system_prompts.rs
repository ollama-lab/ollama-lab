...
pub async fn get_model_system_prompt(state: State<'_, AppState>, model: String) -> Result<Option<String>, Error> {
    ...
    SystemPromptOperator::new(profile_id, model.as_str()).get(&mut *conn).await
}
...
pub async fn set_model_system_prompt(
    ...
    SystemPromptOperator::new(profile_id, model.as_str()).set(&mut *conn, prompt.as_ref().map(|s| s.as_str())).await
}
