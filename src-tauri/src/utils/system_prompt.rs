...
impl<'a> SystemPromptOperator<'a> {
    ...
    pub async fn get(&self, executor: impl Executor<'_, Database = Sqlite>) -> Result<Option<String>, Error> {
        Ok(
            ...
            "#)
                .bind(self.profile).bind(self.model)
    ...
    pub async fn set(&self, executor: impl Executor<'_, Database = Sqlite>, content: Option<&str>) -> Result<Option<String>, Error> {
        let prompt = content 
            .map(|s| s.trim())
