...
export async function getCurrentBranch(sessionId: number): Promise<ChatBubble[]> {
  return await invoke<InternalChat[]>("get_current_branch", { sessionId })
    .then(chats => chats.map(({ id, role, content, imageCount, completed, dateCreated, dateEdited, model, versions, thoughts, thoughtFor }) => ({
      id,
      role: role as Role,
      content,
...
export async function switchBranch(targetChatId: number): Promise<[number | null, ChatBubble[]]> {
  return await invoke<[number | null, InternalChat[]]>("switch_branch", { targetChatId })
    .then(([parentId, chats]) => [parentId, chats.map(({ id, role, content, imageCount, completed, dateCreated, dateEdited, model, versions, thoughts, thoughtFor }) => ({
      id,
      role: role as Role,
      content,
      imageCount: imageCount ?? 0,
